...
SimCase = hysys.SimulationCases.Open([mess.Name,'\mtp 1.10eida2ADJ.hsc']);
...
working_time=8200; cwCost= 0.00002; eleCost=16.8; methaCost=0.33; eleSell=10; ethylene_sell=1.4; propylene_sell=1.5; steamCost=13.28; refrig=7.89; OilCost=13.5;naptha_price=0.55;
%working_time=8200; cwCost= 0.00002; eleCost=16.8; methaCost=0.672; eleSell=15; ethylene_sell=1.4; propylene_sell=1.8; steamCost=13.28; refrig=7.89; OilCost=13.5;
...
%--- fuel oil demants 
opex_table(9)=es.Item(9).HeatFlowValue*1e-6*60*60*working_time*OilCost;
%--- electricity demands
...
OPEX=sum(opex_table);
%% revenue 
revenue=(ms.Item('16').MassFlowValue*60*60*propylene_sell + naptha_price*ms.Item('15').MassFlowValue*60*60)*working_time;
%% OBJECTIVE 
OBJ=revenue-OPEX-aCAPEX

%% LCA CALCULATIONS 
%indi99_steam=0.002617879; indi99_furnace=0.005668631; indi99_ele=0.005817508; indi99_methanol=0.036876948;
meth_recipe=0.134648; steam_recipe=0.044958; furnace_recipe=0.011519; elect_recipe=0.14086;wastewater_recipe=6.2016;steel_recipe=0.48976;
LCA_DATA_SPSHT=hyvalue(hycell(get(hysys.ActiveDocument.Flowsheet.Operations,'Item', 'LCA_DATA'), {'A1', 'A5','A6'})); %a1=diammeter a2=HEIGHT)

%--- single indicator 
steam_lca=(es.Item(15).HeatFlowValue + es.Item(3).HeatFlowValue)/2.6 *60*60*0.001 *steam_recipe*working_time;
FURNACE_LCA=es.Item(9).HeatFlowValue*0.001*60*60*working_time*furnace_recipe;
ELECTRICITY_LCA=-e_diff*277.778*working_time*elect_recipe;
METHANOL_LCA=ms.Item('21').MassFlowValue*60*60*working_time*meth_recipe;
WASTEWATER_LCA=LCA_DATA_SPSHT(1)*60*60*working_time*wastewater_recipe;
STEEL_LCA=LCA_DATA_SPSHT(2)*steel_recipe;

propene_mass=LCA_DATA_SPSHT(3)*60*60*working_time;
Ap1=propene_mass*propylene_sell/revenue;
%% economi Allocation
LCA_TOTAL=((steam_lca+FURNACE_LCA+ELECTRICITY_LCA+METHANOL_LCA+WASTEWATER_LCA) + STEEL_LCA/10)%*Ap1/propene_mass %10 years plant lifetime = x/kg_propene
LCA_PARTS=[steam_lca FURNACE_LCA ELECTRICITY_LCA METHANOL_LCA WASTEWATER_LCA STEEL_LCA/10]*Ap1/propene_mass;
